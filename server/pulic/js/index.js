...
var syy = new _lottery2.default(); //es6兼容处理组件
...
var mix = function mix() {
    ...
    try {
        for (var _iterator2 = minxins[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
var Lottery = function (_mix) {
    ...
    function Lottery() {
        ...
        var _this = _possibleConstructorReturn(this, (Lottery.__proto__ || Object.getPrototypeOf(Lottery)).call(this)); //name 彩种名称识别，cname 彩种名称， issue 当前期号， state 当前状态


        _this.name = name; //对象属性
        _this.cname = cname;
        _this.issue = issue;
        _this.state = state;
        _this.el = ''; //当前选择器
    ...
    _createClass(Lottery, [{
        ...
        value: function initEvent() {
            ...
            (0, _jquery2.default)('.dxjo').on('click', 'li', self.assistHandle.bind(self)); //操作区（大 小 奇偶 清除）
            (0, _jquery2.default)('qkmethod').on('click', 'btn-middle', self.getRandomCode.bind(self)); //随机选则中号码
        }
    }]);
...
exports.default = Lottery; //导出当前接口e
    ...
    _createClass(Base, [{
        ...
        value: function initPlayList() {
            this.play_list.set('r2', {
                ...
                bonus: 90,
                tip: '从01~11中任选6个或多个号码，所选号码与开奖号码五个号相同，即中奖<em class="red">90</em>元',
                name: '任六'
            }).set('r7', {
                bonus: 26,
                tip: '从01~11中任选7个或多个号码，所选号码与开奖号码五个号相同，即中奖<em class="red">26</em>元',
                name: '任七'
            }).set('r8', {
                bonus: 9,
                tip: '从01~11中任选8个或多个号码，所选号码与开奖号码五个号相同，即中奖<em class="red">9</em>元',
                name: '任八'
            }); //用set方法可以避免重复号码
        }
        ...
        value: function initNumber() {
            for (var i = 1; i < 12; i++) {
                this.number.add(('' + i).padStart(2, '0')); //每个字符串保存2个长度，如果长度不够在起始位置添加0
            }
        }
        ...
        //toggleCodeActive 号码选中/取消

    }, {
        key: 'toggleCodeActive',
        value: function toggleCodeActive(e) {
            var self = this;
            var $cur = (0, _jquery2.default)(e.currenTarget);
        ...
        value: function assistHandle(e) {
            ...
            var index = $cur.index(); //选中当前索引
            (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); //把之前的选项都清除
            if (index === 0) {
                (0, _jquery2.default)('.boll-list .btn-boll').addClass('btn-boll-active');
            }
            if (index === 1) {
                (0, _jquery2.default)('.boll-list .btn-boll').each(function (i, t) {
                    if (t, textCountent - 5 > 0) {
        ...
        value: function getRandom(num) {
            var ar = [],
                index = void 0;
            var number = Array.from(this.number); //定义随机空间1~11
            while (num--) {
                index = Number.parseInt(Math.random() * number.length);
                arr.push(number[index]);
                number.splice(index, 1);
            }
            return arr.join(''); //随机生成
        }
...
var Timer = function () {
    ...
    _createClass(Timer, [{
        ...
        value: function countdown(end, update, handle) {
            //end截止时间，update时间更新回调，handle倒计时结束后回调
            var now = new Deta().getTime();
            var self = this;
            if (now - end) {
                handle.call(self); //now>end说明倒计时结束，执行倒计时结束回调
            } else {
                ...
                var m = Math.floor((last_time - d * px_d - h * px_h - m * px_m) / px_s);
                var r = []; //把计算出来的时间保存到r中
                if (d > 0) {
                    r.push('<em>' + d + '</em>\u5929');
                }
                ...
                self.last_time = r.join(''); //把时间保存到last_time
                update.call(self, r.join('')); //用update更新时间
                setTimeout(function () {
                    self.countdown(end, update, handle);
                }, 1000); //每秒调用一次倒计时，不断得达到时间递减的功能。
            }
        }
    }]);
...
var Calculate = function () {
    ...
    _createClass(Calculate, [{
        ...
        //computeCount计算函数
        //{number} active 当前选中号码
        //{string} play_name 当前玩法标识
        // {number} return 计算注数
        value: function computeCount(active, play_name) {
            ...
            if (exist && play_name.at(0) === 'r') {
                count = Calculate.combine(arr, play_name.split('')[1]); //计算注数，完成组合运算。
            }
            return count;
        }
        //computeBonus 奖金范围预测
            ...
            if (play[0] === 'r') {
                ...
                if (min_active > 0) {
                    if (min_active - play[1] >= 0) {
                        ...
                        if (paly[1] - 5 > 0 && active - play[1] >= 0) {
                            arr = new Array(active - 5).fill(0);
                            min = Calculate.combine(arr, play[1] - 5).length;
                        } else {
                            min = active - play[1] > -1 ? 1 : 0; //如果active-play[1]>-1,min=1，否则min=0
                        }
                    }
                } else {
        ...
        value: function combine(arr, size) {
            var allResult = []; //保存运算结果
            (function f(arr, size, result) {
                var arrLen = arr.length; //判断当前数组的传入长度
                if (size > arr) {
                    //如果传入的长度小于size
                    return; //递归运算截止
                }
                if (size === arrLen) {
                    allResult.push([].concat(result, arr)); //把最后的数组保存在allResult中
                } else {
                    for (var i = 0; i < arrLen; i++) {
                        var newResult = [].concat(result); //这个循环就是不断生成新数组的过程。
                        newResult.push(arr[i]); //新增加一个数组，并把结果保存
                        if (size === 1) {
                            allResult.push(newResult); //保存新数组  
                        } else {
                            var newArr = [].concat(arr);
                            newArr.splice(0, i + 1); //把新的数组截一段数组出来，形成下一个子集参与运算
                            f(newArr, size - 1, newResult); //再次递归的时候会把当前size-1，在新的子集上运算并保存上次的结果。
                        } //在es6中立即执行函数也需要命名（f)
                    }
                }
            })(arr, size, []); //立即执行函数，运用递归的运算方式，实现组合。
        }
    }]);
...
exports.default = Calculate; //导出当前接口
...
var Interface = function () {
    ...
    _createClass(Interface, [{
        ...
        value: function getOmit(issue) {
            var self = this;
            return new Promise(function (resolve, reject) {
                //resolve已完成，reject已失败。
                _jquery2.default.ajax({
                    url: '/get/omit',
                    data: {
                        issue: issue //前后端约定数据格式
                    },
                    dataType: 'json',
                    success: function success(res) {
                        self.setOmit(res.data); //保存当前数据在当前对象上
                        resolve.call(self, res); //执行resolve
                    }, //表示通信成功
                    error: function error(err) {
                        reject.call(err); //如果失败，执行reject，取消下一把
                    } //表示通信失败
                });
            }); //方便异步操作
        }
        ...
        value: function getOpenCode(issue) {
            var self = this; //通过声明this对象的引用，解决this指向不明确问题
            return new Promise(function (resolve, reject) {
                _jquery2.default.ajax({
                    url: '/get/opencode',
                    data: {
...
exports.default = Interface; //导出当前接口
